...
class GTF:
    def __init__(self, seqname=None, source=None, feature=None, start=None, end=None, score=None, strand=None, frame=None, attribute=None):
        ...
        try:
            self.attribute = dict(filter(None,[x.split() for x in self.attribute.split(';')]))
    ...
    def __str__(self):
        att = ('; ').join(['{} {}'.format(k,v) for k,v in self.attribute.items()])
        return '{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}'.format(self.seqname, self.source, self.feature, self.start, self.end, self.score, self.strand, self.frame, att)
